...
namespace XtreamDataAccess
{
    public sealed class SqlHelper
    {
        ...
        public static int ExecuteNonQueryImages(string connetctionString, byte[] byteArray, string commandText)
        {
            SqlCommand cmd = new SqlCommand();
            cmd.Parameters.Clear();
            cmd.Parameters.AddWithValue("@picture", byteArray);
            cmd.CommandText = commandText;

            using (SqlConnection cn = new SqlConnection(connetctionString))
            {
                cn.Open();
                cmd.Connection = cn;
